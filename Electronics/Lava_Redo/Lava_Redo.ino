...
// -- PHOTO SENSOR -- //
#define DARKNESS_THRESH 500 // value below which is considered "dark". This will need to be tuned.
...
int lavaIdx = 0;
uint8_t lavaColours[NUM_COLORS][3] = {
 {255, 65, 0},
 {255, 60, 0},
 {255, 55, 0},
 {255, 50, 0},
...
uint8_t currColours[NUM_LEDS][3];
uint8_t newColours[NUM_LEDS][3];
...
void checkBrightnessChange()
{
  ...
  if (currLight != lightOn  && !transitioning ) {
...
void lavaflow()
{
    ...
    {
        int r = random(NUM_COLORS);

        int red = lavaColours[r][0];    

        int green = lavaColours[r][1];
    
        int blue = lavaColours[r][2];
